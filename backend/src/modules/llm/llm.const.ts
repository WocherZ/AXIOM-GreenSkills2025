...
const CREATE_SLIDES_ANSWER_EXAMPLE =
  '```json\n[{"title": "..."}, {"title": "..."}]\n```';
...
export const CREATE_SLIDES_USER_PROMPT_TEMPLATE =
  'Напиши заголовки слайдов для презентации на языке {presentation_language}, состоящую из {slides_number} слайдов, на тему: {topic}.';

// 2. Создание контента слайдов
const CREATE_SLIDES_CONTENT_ANSWER_EXAMPLE =
  '```json\n[{"title": "...", "text": "...", "list": ["..."]}]\n```';
...
export const CREATE_SLIDES_CONTENT_USER_PROMPT_TEMPLATE =
  "Напиши содержимое для презентации на тему: {topic}. Язык презентации: {presentation_language}. Для каждого заголовка из следующего списка составь содержимое: {slides_title_json}. Убедись, что ты используешь поле 'list' только при необходимости. При создании думай шаг за шагом, продумывай что будешь писать.";
