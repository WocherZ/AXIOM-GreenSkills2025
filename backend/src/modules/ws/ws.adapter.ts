...
export class WsAdapter extends IoAdapter {
  ...
  public createIOServer(port: number, options?: ServerOptions): Server {
    ...
    const WS_PING_INTERVAL = parseInt(
      this.config.getOrThrow<string>('WS_PING_INTERVAL'),
      10,
    );
    const WS_PING_TIMEOUT = parseInt(
      this.config.getOrThrow<string>('WS_PING_TIMEOUT'),
      10,
    );
    ...
    const server: Server = super.createIOServer(port || WS_PORT, {
      ...
      cors: CorsMiddleware({
        allowed_origins: JSON.parse(
          this.config.getOrThrow<string>('CORS_ALLOWED_ORIGINS'),
        ),
        allowed_methods: JSON.parse(
          this.config.getOrThrow<string>('CORS_ALLOWED_METHODS'),
        ),
        allowed_paths: JSON.parse(
          this.config.getOrThrow<string>('CORS_ALLOWED_PATHS'),
        ),
    ...
    server.use((socket, next) => {
      ...
      if (access_token?.length) {
        try {
          const { token_type } = this.jwt.verify(access_token);
          ...
          return next(access_token_expired_signature(e));
        }
      }
